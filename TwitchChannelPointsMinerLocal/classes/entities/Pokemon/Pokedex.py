...
class Pokedex(object):
    ...
    def _clean_pokedex_name(pokemon):
        if Pokedex._check_valid(pokemon):
            return pokemon

        if "(Blade)" in pokemon:
            new_name = pokemon.replace("Blade", "Shield")
    ...
    def legendary(self, pokemon):
        poke_name = self._get_pokemon_name(self.clean_name(pokemon))
    ...
    def clean_name(pokemon):
        ...
        if pokemon.startswith("Aegislash"):
            pass
